        ...
        if(uart_on)
        {
            if(uart_read_en)
            {
                ...
                if(uart_on && uart_read_en)
                {
                    ...
                    if(ret == -1)
                     {
                            dialog_pthread_read.setWindowTitle("uart read fialed");
                            dialog_pthread_read.exec();
                     }else
                    {
                           ;
                    }
                ...
                if(strlen(uart_buf_write))
                {
                    if(uart_send_auto)
                    {
                        ...
                        if(ret == -1)
                          ...
                          dialog_pthread.setWindowTitle("uart write failed");
                          dialog_pthread.exec();
...
int uart_set(int uart_fd,int baude,int c_flow,int bits,int parity,int stop,Ui::uart *ui)
{
    ...
    {
        QDialog dialog ;
        dialog.setWindowTitle("tcsetattr failed");
        dialog.exec();
        return -1;
    }
    ...
    if(!pthread_on)
    {
        ...
        {
            QDialog dialog ;
            dialog.setWindowTitle("fial to pthread_create_read");
            dialog.exec();
            exit(1);
        }
        ...
        {
            QDialog dialog ;
            dialog.setWindowTitle("fail to pthread_create_write");
            dialog.exec();
            exit(1);
        }
        pthread_on =1;
    ...
    uart_on = 1;
    QDialog dialog ;
    dialog.setWindowTitle("uart_set is ok");
    dialog.exec();
    return 0;
}
...
int uart_read(int fd,char *r_buf,size_t len,Ui::uart *ui)
{
    ...
    {
           QDialog dialog ;
           dialog.setWindowTitle("read error");
           dialog.exec();
           return -1;
    }
           return cnt;
...
int uart_write(int fd,const char *w_buf,size_t len)
{
    ...
    if(uart_send_en == 1)
    {
      ...
      {
           QDialog dialog ;
           dialog.setWindowTitle("write error!!!");
           dialog.exec();
           return -1;
      }
...
int uart_main(const char * file_uart,int baude,int c_flow,int bits,int  parity,int stop,Ui::uart *ui)
{
    ...
    {
        QDialog dialog;
        dialog.setWindowTitle("fail to open file!");
        dialog.exec();
        return -1;
    }
    ...
    {
            QDialog dialog;
            dialog.setWindowTitle("fail to set uart");
            dialog.exec();
            return -1;
    }
    return 0;
...
void uart::on_uart_send_clicked()
{
    ...
    if(strcpy(uart_buf_write,ui->uart_send_text->toPlainText().toStdString().c_str()) !=0)
    {
        QDialog dialog;
        dialog.setWindowTitle(tr("uart_send_text error"));
        dialog.exec();
        return ;
    }
    ...
    {
        QDialog dialog;
        dialog.setWindowTitle(tr("please open uart_file"));
        dialog.exec();
        return ;
    }
    else
...
void uart::on_uatr_on_clicked()
{
    ...
    {
        QDialog dialog;
        dialog.setWindowTitle(tr("please chosse start!!!"));
        dialog.exec();
        return ;
    }
    ...
    {
        QDialog dialog;
        dialog.setWindowTitle(tr("The file has been opened !!!"));
        dialog.exec();
        return ;
